...
# Системный каталог меню
install -m 644 /dev/null /usr/share/applications/anomalynet.desktop 2>/dev/null || true
echo "$DESKTOP_CONTENT" > /usr/share/applications/anomalynet.desktop
update-desktop-database /usr/share/applications/ 2>/dev/null || true
ok "Ярлык добавлен в меню приложений"

# Рабочий стол текущего (или sudo) пользователя
REAL_USER="${SUDO_USER:-$USER}"
REAL_HOME=$(eval echo "~$REAL_USER")
for DESKTOP_DIR in "$REAL_HOME/Desktop" "$REAL_HOME/Рабочий стол" "$REAL_HOME/Рабочий_стол"; do
    if [ -d "$DESKTOP_DIR" ]; then
        echo "$DESKTOP_CONTENT" > "$DESKTOP_DIR/AnomalyNet IDS.desktop"
        chmod +x "$DESKTOP_DIR/AnomalyNet IDS.desktop" 2>/dev/null || true
        chown "$REAL_USER:$REAL_USER" "$DESKTOP_DIR/AnomalyNet IDS.desktop" 2>/dev/null || true
        ok "Ярлык добавлен на рабочий стол: $DESKTOP_DIR"
        break
    fi
done
...
echo -e "  Ярлык         : ${CYAN}Меню приложений → AnomalyNet IDS${NC}"
echo ""
