...
> **Зачем права администратора?** На Windows захват трафика (Npcap) и блокировка
> IP (брандмауэр Windows / netsh) требуют повышения прав. Поэтому и ярлык на
> рабочем столе, и **сам `launch.bat` автоматически запрашивают повышение через
> UAC** (а «Запустить панель» из трея тоже стартует панель с правами админа) —
> при запуске появляется UAC-запрос. Без повышения прав панель всё равно
> работает, но в топбаре показываются **⚠ Нет захвата / ⚠ Нет фаервола**, и эти
> функции недоступны.
...
## AnomalyNet Control (приложение в трее)

Вместе с панелью ставится лёгкое мини-приложение в системном трее (консоль не
нужна). По правому клику на значке:

- **Открыть панель** в браузере
- **Запустить / Перезапустить / Остановить** панель
- **Живые метрики** (CPU / RAM / события) в подсказке и заголовке меню
- Два **независимых переключателя автозапуска** — панели и самого приложения
  (по умолчанию при входе стартуют оба; любой можно отключить из меню)

Работает отдельным процессом и не требуется для работы панели. Запуск вручную:
`tray.vbs` (Windows) / `bash tray.sh` (Linux). Основан на `pystray` + `Pillow`
...
├── launch.sh · launch.bat · launch.vbs   Лаунчеры панели (ярлыки и автозапуск)
└── tray.sh · tray.bat · tray.vbs   Лаунчеры трей-приложения (AnomalyNet Control)
```
Код трей-приложения — в `backend/app/tray/` (`main.py`, `controller.py`, `autostart.py`).
