...
> IP (брандмауэр Windows / netsh) требуют повышения прав. Поэтому ярлык на рабочем
> столе, создаваемый `install.bat`, помечен флагом **«Запуск от имени
> администратора»** — при запуске появляется UAC-запрос. Без повышения прав
> приложение всё равно работает, но в топбаре показываются **⚠ Нет захвата /
> ⚠ Нет фаервола**, и эти функции недоступны.
