...
AnomalyNet автоматически строит карту устройств локальной сети и использует её для маршрутизации трафика к нужной модели обнаружения.
...
**1. ARP-сканирование** — `NetworkScanner` периодически рассылает ARP-запросы по подсети, собирая MAC-адреса и IP всех активных устройств.

**2. Классификация устройств** — по MAC-префиксу (OUI-база) определяется производитель, а затем тип устройства: `iot_camera`, `iot_sensor`, `pc_windows`, `phone`, `router` и др. На основе имени хоста и открытых портов классификатор уточняет тип.

**3. DeviceTracker** — ведёт живой реестр устройств: IP, MAC, тип, имя хоста, время первого/последнего появления, статус онлайн, счётчик алертов и байты трафика за последние 5 минут.

**4. Device-aware routing (пресет `auto`)** — при выборе режима "Авто" каждый поток маршрутизируется по типу устройства-источника:

| Тип устройства | Pipeline |
|---|---|
| IoT-камеры, датчики, лампочки, роутеры | Advanced Cascade (Stage 1 → Stage 3, 46 IoT-признаков) |
| ПК, ноутбуки, смартфоны, серверы | General Network (Stage 1 → Stage 2, CICIDS 2017) |
| Неизвестные устройства | Advanced Cascade (консервативно) |

Это позволяет использовать специализированные модели для каждого типа трафика, избегая ложных срабатываний при смешанной домашней сети.

### Модели трафика

Каждый тип устройства порождает предсказуемые паттерны трафика, под которые заточены соответствующие модели:

| Устройство | Типичные потоки | Модель обучена на |
|---|---|---|
| IoT-камера | RTSP/RTP стриминг, NTP, ICMP | CIC IoT Dataset 2024 |
| ПК/ноутбук | HTTP/S браузинг, SSH, FTP, DNS | CICIDS 2017 |
| Смартфон | HTTPS API, Push-уведомления | CICIDS 2017 |

### Threat model: что система обнаруживает

IDS работает в **host-based** режиме — захватывает трафик на интерфейсе машины, где установлена. Это значит, что она видит и анализирует:

- **Входящие атаки** на машину с IDS (PortScan, BruteForce, DoS от устройств в той же сети)
- **Исходящие аномалии** с заражённых устройств (Mirai-сканирование, C2-трафик ботнетов)
- **Атаки из той же сети** — взломанный IoT-гаджет, заражённый ПК или чужой в гостевой WiFi

Для мониторинга **всего** трафика сети (не только своего) IDS необходимо развернуть на шлюзе (роутере с OpenWrt, Raspberry Pi в режиме моста) или настроить зеркалирование портов на управляемом коммутаторе.

### Карта сети в интерфейсе

Вкладка **«Карта сети»** показывает:
- Интерактивный D3-граф всех устройств с иконками по типу
- Боковую панель с детальной информацией: производитель, тип, трафик, история алертов
- Статусы: онлайн / офлайн / подозрительное (красная подсветка при аномальной активности)
- Действия: переименовать, добавить в белый список, сбросить счётчик алертов

В mock-режиме вкладка отображает 7 тестовых устройств (роутер, 2 камеры, датчик, ПК, телефон, умная лампа) без ARP-сканирования.

### Структура модуля

```
backend/app/discovery/
├── models.py       — DeviceInfo dataclass, таблица типов устройств (14 типов)
├── oui.py          — OUI lookup: MAC-префикс → производитель (из config/oui.json)
├── classifier.py   — guess_device_type(vendor, hostname, ports) → тип
├── scanner.py      — NetworkScanner: ARP-скан (Scapy) + mock-режим
└── tracker.py      — DeviceTracker: реестр, история алертов, трафик за 5 мин
```
